...
CREATE TABLE controls_users(
    ...
    FOREIGN KEY (id_responsibleUser_control_users) REFERENCES users(id_user) ON DELETE CASCADE ON UPDATE CASCADE
);
...
/* TEMP OR TRASH. SECURITY BACKUP */

CREATE TABLE controls_users_trash(
    id_control_users SERIAL,
...
CREATE TABLE products_controls_users_trash(
    id_control_users INT NOT NULL,
    id_product INT NOT NULL,
    stock_product_control_users INT DEFAULT 0 CHECK (stock_product_control_users >= 0),
    sale_product_control_users INT DEFAULT 0 CHECK (sale_product_control_users >= 0),
    FOREIGN KEY (id_control_users) REFERENCES controls_users_trash(id_control_users) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (id_product) REFERENCES products(id_product) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE articles_controls_users_trash( 
    id_control_users INT NOT NULL,
    id_article INT NOT NULL,
    stock_article_control_users INT DEFAULT 0,
    exists_article_control_users BOOLEAN DEFAULT FALSE,
...
CREATE TABLE observations_trash(
    id_observation SERIAL,
    id_control_users INT NOT NULL,
    details_observation TEXT NOT NULL,
    income_observation REAL DEFAULT 0 CHECK (income_observation >= 0),
